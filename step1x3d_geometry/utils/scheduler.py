...
def parse_optimizer(config, model):
    if hasattr(config, "params"):
        ...
        # If model is a list of parameters, use it directly.
        # Otherwise, assume it's an nn.Module and get parameters from it.
        if isinstance(model, list):
            params = model
        elif hasattr(config, "only_requires_grad") and config.only_requires_grad:
