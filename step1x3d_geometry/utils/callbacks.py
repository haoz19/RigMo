...
class S3SyncCallback(Callback):
    """Syncs the trial directory to S3 after checkpoint saves using rclone."""

    def __init__(self, local_dir: str, s3_path: str, exclude_patterns=None):
        super().__init__()
    ...
    def _do_sync(self, reason: str = ""):
        ...
        if not _shutil.which("rclone"):
            rank_zero_warn("S3SyncCallback: rclone not found, skipping sync")
            return
        cmd = ["rclone", "copy", self.local_dir, self.s3_path, "--transfers", "8", "-v"]
        for pat in self.exclude_patterns:
            cmd += ["--exclude", pat]
        self._sync_count += 1
        tag = f"[S3Sync #{self._sync_count}]"
        print(f"{tag} syncing {self.local_dir} -> {self.s3_path} ({reason})")
        try:
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=600)
            if result.returncode == 0:
                print(f"{tag} sync complete")
            else:
                rank_zero_warn(f"{tag} rclone failed (rc={result.returncode}): {result.stderr[:500]}")
        except subprocess.TimeoutExpired:
            rank_zero_warn(f"{tag} rclone timed out after 600s")
        except Exception as e:
            rank_zero_warn(f"{tag} sync error: {e}")

    @rank_zero_only
    def on_train_epoch_end(self, trainer, pl_module):
        self._do_sync(reason=f"epoch {trainer.current_epoch} end")

    @rank_zero_only
    def on_save_checkpoint(self, trainer, pl_module, checkpoint):
        self._do_sync(reason=f"checkpoint at step {trainer.global_step}")

    @rank_zero_only
    def on_train_end(self, trainer, pl_module):
        self._do_sync(reason="training finished")
